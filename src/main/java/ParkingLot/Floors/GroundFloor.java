...
import static ParkingLot.Constants.ParkingConstants.TOTAL_TRUCK_SLOTS;
...
public class GroundFloor implements Floor {
    ...
    public GroundFloor(VehicleType type) {
        this.vehicleType = type;
        slotMap = new HashMap<>(TOTAL_TRUCK_SLOTS);
        availableSlots = new TreeSet<>();
        for(int i = 1; i <= TOTAL_TRUCK_SLOTS; i++){
    ...
    public boolean freeSlot(int slotNumber) {
        if (slotNumber < 0 || slotNumber >= TOTAL_TRUCK_SLOTS) {
            return false;
        }
